...
class LibraryModel:
    ...
    def has_umi_annotation(self):
        return self._has_annotation(longbow.utils.constants.READ_RAW_UMI_TAG) or \
               self._has_annotation(longbow.utils.constants.READ_UMI_TAG)

    @property
    def has_cell_barcode_annotation(self):
        return self._has_annotation(longbow.utils.constants.READ_RAW_BARCODE_TAG) or \
               self._has_annotation(longbow.utils.constants.READ_BARCODE_TAG)
    ...
    def build(self):
        ...
        self.hmm.add_transition(all_states['random-start'],
                                all_states[f"{self.cdna_model['structure'][0]}-start"], 0.5)
        self.hmm.add_transition(all_states['random-start'],
                                all_states[f"{self.array_model['structure'][0]}-start"], 0.5)
            ...
            if type(adapter_def) is str:
                if adapter_def == RANDOM_SEGMENT_NAME:
                    ...
                    if next_adapter_name is not None:
                        self.hmm.add_transition(all_states[f'{adapter_name}-end'],
                ...
                if segment_type == FIXED_LENGTH_RANDOM_SEGMENT_TYPE_NAME:
                    ...
                    for op in ['M', 'I']:
                        self.hmm.add_transition(all_states[f'{adapter_name}:{op}{hpr_length}'],
                                                all_states['random:RDB'], 0.01)
                        self.hmm.add_transition(all_states[f'{adapter_name}:{op}{hpr_length}'],
                                                all_states['random-end'], 0.01)
    ...
    def _create_array_model(self):
        ...
        for adapter_name in self.array_model['structure']:
            adapter_hmm = ModelBuilder.make_global_alignment_model(self.array_model['adapters'][adapter_name],
                                                                   adapter_name)
        ...
        for adapter_name, adapter_def in self.cdna_model['adapters'].items():
            ...
            if type(adapter_def) is str:
                if adapter_name in self.named_random_segments:
                    ...
                    adapter_hmm = ModelBuilder.make_global_alignment_model(self.cdna_model['adapters'][adapter_name],
                                                                           adapter_name)
                ...
                if segment_type == FIXED_LENGTH_RANDOM_SEGMENT_TYPE_NAME:
                    adapter_hmm = ModelBuilder.make_fixed_length_random_segment(adapter_name,
                                                                                list(adapter_def.values())[0])
                elif segment_type == HPR_SEGMENT_TYPE_NAME:
                    base, hpr_length = list(adapter_def.values())[0]
    ...
    def _assert_valid_model_json_data(json_data):
        ...
        if len(missing_fields) > 0:
            message = f"ERROR: JSON model is missing one or more required fields: {','.join(missing_fields)}.  " \
    ...
    @staticmethod
    def _assert_model_segments_are_in_collection(model, segments_name, collection_name, model_type):
        not_present = []
        ...
        if len(not_present) > 0:
            message = f"ERROR: {model_type} model has the following segments defined in {segments_name}, " \
    ...
    def validate_model(self):
        ...
        required_array_model_fields = ["description", "version", "structure", "adapters", "deprecated", "name"]
        required_cdna_model_fields = required_array_model_fields + ["named_random_segments", "coding_region",
                                                                    "annotation_segments"]
        # Make sure our models have required fields:
        LibraryModel._assert_required_model_fields_are_present(self.array_model, required_array_model_fields, "Array")
        LibraryModel._assert_required_model_fields_are_present(self.cdna_model, required_cdna_model_fields, "cDNA")

        ##############################################################################
        # Checks on general sub-model structure:

        # Array model:
        LibraryModel._assert_model_segments_are_in_collection(self.array_model, "adapters", "structure", "Array")
        LibraryModel._assert_model_segments_are_in_collection(self.array_model, "structure", "adapters", "Array")

        # cDNA Model:
        LibraryModel._assert_model_segments_are_in_collection(self.cdna_model, "adapters", "structure", "cDNA")
        LibraryModel._assert_model_segments_are_in_collection(self.cdna_model, "structure", "adapters", "cDNA")

        for seg_def in ["named_random_segments", "coding_region", "annotation_segments"]:
            LibraryModel._assert_model_segments_are_in_collection(self.cdna_model, seg_def, "structure", "cDNA")
            LibraryModel._assert_model_segments_are_in_collection(self.cdna_model, seg_def, "adapters", "cDNA")
